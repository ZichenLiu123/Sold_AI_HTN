...
export async function GET(req: Request) {
  ...
  if (capture) {
    try {
      ...
      return NextResponse.json(
        {
          error: error instanceof Error ? error.message : "Could not read Facebook Selling.",
        },
        { status: 500 }
      );
    }
  ...
  if (peek) {
    try {
      ...
      return NextResponse.json(
        {
          error: error instanceof Error ? error.message : "Could not read Facebook inbox.",
          threads: [],
          latest: null,
        },
        { status: 500 }
...
export async function POST() {
  return NextResponse.json(startFacebookMonitor());
}

export async function DELETE() {
  return NextResponse.json(stopFacebookMonitor());
}
